...
describe("InputAsumsiParameter.vue", () => {
  ...
  it("should call the onMounted fetch methods", async () => {
    const fetchMesinByIdSpy = jest.spyOn(wrapper.vm, "fetchMesinById");
    const fetchStatusRealisasiByIdSpy = jest.spyOn(
      wrapper.vm,
      "fetchStatusRealisasiById"
    );
    ...
    const fetchComboBahanBakarSpy = jest.spyOn(
      wrapper.vm,
    ...
    expect(fetchStatusRealisasiByIdSpy).toHaveBeenCalledTimes(0);
    expect(fetchCheckIntegrasiSpy).toHaveBeenCalledTimes(0);
    expect(fetchAsumsiParameterSpy).toHaveBeenCalledTimes(0);
