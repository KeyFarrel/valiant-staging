...
describe('GraphicOpexc_Nphr.vue', () => {
  let wrapper: any;
  ...
  beforeEach(() => {
    ...
    grafikServiceMock.getGraphicOpexC = jest.fn().mockResolvedValue({
      success: true,
      data: [{
        grafik: [{ data: { nphr: 2000, value_c: 5000 }, kode_jenis_kit: 'PLTU', nama_mesin: 'Mesin A' }],
        legend: [{ label: 'PLTU', color: '#FF5656' }],
        average_nphr: 1800,
        average_value_c: 4000,
        average_ipp_nphr: 1600,
        average_ipp_valuec: 3000
      }]
    });

    wrapper = shallowMount(GraphicOpexc_Nphr, {
      global: {
        components: {
          Empty,
          ShimmerLoading,
  ...
  it('should call fetchInitialPembangkit and getDataGraph on mounted', async () => {
    ...
    expect(grafikServiceMock.getGraphicOpexC).toHaveBeenCalledTimes(0);
  });
  ...
  it('should display the empty state when no data is available', async () => {
    grafikServiceMock.getGraphicOpexC.mockResolvedValue({
      success: true,
      data: [{ grafik: null, legend: [], average_nphr: 1800, average_value_c: 4000, average_ipp_nphr: 1600, average_ipp_valuec: 3000 }]
    });
    await wrapper.vm.getDataGraph();
