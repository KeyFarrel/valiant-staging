...
describe('GraphicCapex_Ncf.vue', () => {
  ...
  beforeEach(() => {
    ...
    grafikServiceMock.getGraphicAnalitikCF = jest.fn().mockResolvedValue({
      success: true,
      data: [{
        grafik: [{ data: { ncf: 80, capex: 1500 }, kode_jenis_kit: 'PLTU', nama_mesin: 'Mesin B' }],
        legend: [{ label: 'PLTU', color: '#FF5656' }],
        average_pln_ncf: 85,
        average_pln_capex: 1200,
        average_ipp_ncf: 80,
        average_ipp_capex: 1100
      }]
    });

    wrapper = shallowMount(GraphicCapex_Ncf, {
      global: {
        components: {
          Empty,
          ShimmerLoading,
  ...
  it('should call fetchInitialPembangkit and getDataGraph on mounted', async () => {
    ...
    expect(grafikServiceMock.getGraphicAnalitikCF).toHaveBeenCalledTimes(0);
  });
  ...
  it('should display the empty state when no data is available', async () => {
    grafikServiceMock.getGraphicAnalitikCF.mockResolvedValue({
      success: true,
      data: [{ grafik: null, legend: [], average_pln_ncf: 85, average_pln_capex: 1200, average_ipp_ncf: 80, average_ipp_capex: 1100 }]
    });
    await wrapper.vm.getDataGraph();
