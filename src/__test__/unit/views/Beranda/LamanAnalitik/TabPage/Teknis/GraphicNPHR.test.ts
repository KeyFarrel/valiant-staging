...
describe('GraphicNPHR.vue', () => {
  ...
  beforeEach(() => {
    ...
    grafikServiceMock.getGraphicTeknisNPHR = jest.fn().mockResolvedValue({
      success: true,
      data: {
    ...
    wrapper = shallowMount(GraphicNPHR, {
      global: {
        components: {
          Empty,
          ShimmerLoading,
  ...
  it('should call fetchInitialPembangkit and getDataGraph on mounted', async () => {
    ...
    expect(grafikServiceMock.getGraphicTeknisNPHR).toHaveBeenCalledTimes(0);
  });
  ...
  it('should display the empty state when no data is available', async () => {
    grafikServiceMock.getGraphicTeknisNPHR.mockResolvedValue({
      success: true,
