...
describe('GraphicNCF.vue', () => {
  ...
  beforeEach(() => {
    ...
    grafikServiceMock.getGraphicTeknisNCF = jest.fn().mockResolvedValue({
      success: true,
      data: {
        legend: [{ label: 'PLTU', color: '#0099AD' }],
        data: [{ data: { tahun: '2023', value: 75 }, kode_jenis_kit: 'PLTU', nama_mesin: 'Mesin 1' }]
      }
    });

    wrapper = shallowMount(GraphicNCF, {
      global: {
        components: {
          Empty,
          ShimmerLoading,
  ...
  it('should call fetchInitialPembangkit and getDataGraph on mounted', async () => {
    ...
    expect(grafikServiceMock.getGraphicTeknisNCF).toHaveBeenCalledTimes(0);
  });
  ...
  it('should display the empty state when no data is available', async () => {
    grafikServiceMock.getGraphicTeknisNCF.mockResolvedValue({
      success: true,
