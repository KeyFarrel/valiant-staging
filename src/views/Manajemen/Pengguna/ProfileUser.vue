<template>
  ...
  <ModalNotification :show-modal="isSuccess" :animation-data="successJson" :title="'Berhasil Mengganti Password'"
    :subtitle="'Anda telah berhasil mengganti password'" />
        ...
        <div class="flex flex-col space-y-5 w-[60%]">
          <div class="relative flex flex-col space-y-1">
            ...
            <TextField id="oldPassword" :type="showOldPassword ? 'text' : 'password'" @on-copy="preventCopyPaste"
              @on-paste="preventCopyPaste" placeholder="Masukkan password lama anda" class="text-sm"
              v-model="oldPassword" />
          ...
          <div class="relative flex flex-col space-y-1">
            ...
            <TextField @on-input="verifyRequirementPassword" @on-copy="preventCopyPaste" @on-paste="preventCopyPaste"
              id="newPassword" :type="showNewPassword ? 'text' : 'password'" placeholder="Masukkan password baru anda"
              class="text-sm" v-model="newPassword" />
          ...
          <div class="relative flex flex-col space-y-1">
            ...
            <TextField @on-input="verifyMatchPassword" @on-copy="preventCopyPaste" @on-paste="preventCopyPaste"
              id="confirmNewPassword" :type="showConfirmNewPassword ? 'text' : 'password'"
              placeholder="Masukkan konfirmasi password baru anda" class="text-sm" v-model="confirmNewPassword" />
...
const changePassword = async () => {
  if (!isPasswordMatched.value || !hasMinLength.value || !hasNumber.value || !hasUppercase.value || !hasLowercase.value || !hasSymbol.value) {
    ...
    try {
      isLoading.value = true;
      await authService.changePassword(data.value.email, oldPassword.value, newPassword.value);
