...
const props = defineProps<{
  itemsPembangkit:
  { id: string; name: string, power?: string }[]
  itemsDayaMampu:
  { id: string; name: string }[]
  itemsDaya:
  { id: string; daya: string; satuan: string }[]
  title: string;
  yearRange: number[]
}>();
...
async function getDataGraph() {
  try {
    ...
    const response: BaseResponse<ResComponent> = await grafikService.getGraphicBiaya(param);
    if (response.success) {
      const data = response.data
      graphData.value.series = []
      graphData.value.values = [];
      graphData.value.legends = []
      graphData.value.isEmpty = data.data.length === 0;
...
<template>
  <div class="flex flex-col h-full px-6 pt-4 space-y-1.5 pb-4 mt-4 bg-white rounded-lg">
    <div class="flex">
        ...
        <div class="flex flex-col space-y-3">
          ...
          <div class="flex flex-col space-y-0.5">
            ...
            <VueDatePicker v-model="filter.periode" placeholder="Pilih Periode" :format-locale="id"
              :yearRange="props.yearRange" :range="true" :enable-time-picker="false" :hideInputIcon="false"
              :clearable="false" :showNowButton="true" :year-picker="true" :month-change-on-scroll="false"
              :teleport="true" />
          </div>
    ...
    <div class="flex">
      ...
      <div v-else>
        <DynamicScatterPlotVertiLine v-if="!graphData.isEmpty" :series="graphData.series"
          ...
          :xData="{ name: 'Tahun', satuan: '' }" :yData="{ name: 'Biaya B + D', satuan: 'Rp (Juta)' }"
