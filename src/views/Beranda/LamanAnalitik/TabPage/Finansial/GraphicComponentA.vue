  ...
  try {
    ...
    if (response.success) {
      ...
      data.legend?.map((item) => {
        ...
        data.data?.map(graph => {
          graphData.value.years.push(
            parseInt(graph.data.tahun)
          )
          graphData.value.values.push(graph.data.value)
          if (graph.kode_jenis_kit === item.label) {
            scatterTemplate.data.push([parseInt(graph.data.tahun),
            graph.data.value, 5, graph.nama_mesin])
          }
        }
        );
        graphData.value.series.push(scatterTemplate)
    ...
    <div class="flex">
      ...
      <div v-else>
        <DynamicScatterPlotVertiLine v-if="!graphData.isEmpty" :series="graphData.series"
          ...
          :xData="{ name: 'Tahun', satuan: '' }" :yData="{ name: 'Biaya A', satuan: 'Rp (Juta) ' }"
