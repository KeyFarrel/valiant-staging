...
import type { BaseResponse, ResCapexNcf } from "@/views/Beranda/LamanAnalitik/TypeFinansial";
...
async function getDataGraph() {
  try {
    ...
    const response: BaseResponse<ResCapexNcf[]> = await grafikService.getGraphicAnalitikCF({
      // kode_jenis_kit: filter.value.kategoriPembangkit ? filter.value.kategoriPembangkit.toString() : "",
    ...
    if (response.success) {
      ...
      graphData.value.pln.x = data.average_pln_ncf
      graphData.value.pln.y = data.average_pln_capex
      graphData.value.ipp.x = data.average_ipp_ncf
      ...
      data.legend?.map((item, index) => {
        ...
        data.grafik?.map(graph => {
          if (graph.kode_jenis_kit === item.label) {
            scatterTemplate.data.push([graph.data.ncf, graph.data.capex, 5, graph.nama_mesin])
            graphData.value.source?.push([graph.data.ncf, graph.data.capex])
          }
        })
  ...
  <div class="flex flex-col h-full px-6 pt-4 pb-4 mt-4 bg-white rounded-lg">
    ...
    <div v-else>
      <DynamicScatterPlot v-if="!graphData.isEmpty" :source="graphData.source" :series="graphData.series"
        :legends="graphData.legends" :pln="graphData.pln" :ipp="graphData.ipp" :xData="{ name: 'NCF', satuan: '%' }"
        :yData="{ name: 'Capex', satuan: 'Rp (Juta)' }" :data-zoom="graphData.dataZoom" />
