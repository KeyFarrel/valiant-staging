...
import type { BaseResponse, ResOpexNphr } from "@/views/Beranda/LamanAnalitik/TypeFinansial";
...
async function getDataGraph() {
  try {
    ...
    const response: BaseResponse<ResOpexNphr[]> = await grafikService.getGraphicOpexC({
    ...
    if (response.success) {
      ...
      graphData.value.pln.x = data.average_nphr
      graphData.value.pln.y = data.average_value_c
      graphData.value.ipp.x = data.average_ipp_nphr
      graphData.value.ipp.y = data.average_ipp_valuec
      data.legend?.map((item, index) => {
        ...
        data.grafik?.map(graph => {
          if (graph.kode_jenis_kit === item.label) {
            scatterTemplate.data.push([graph.data.nphr, graph.data.value_c, 5, graph.nama_mesin])
            graphData.value.source?.push([graph.data.nphr, graph.data.value_c])
          }
        })
  ...
  <div class="flex flex-col h-full px-6 pt-4 pb-4 mt-4 bg-white rounded-lg">
    ...
    <div v-else>
      <DynamicScatterPlot v-if="!graphData.isEmpty" :source="graphData.source" :series="graphData.series"
        ...
        :xData="{ name: 'NPHR', satuan: 'kcal/KWh' }" :yData="{ name: 'Opex C', satuan: 'Rp (Juta)' }"
