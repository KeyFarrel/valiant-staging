<template>
          ...
          <div class="flex flex-col space-y-3">
            <div class="flex flex-col space-y-1">
              ...
              <p class="text-xs text-textDisabledColor">
                Template ini wajib digunakan saat pertama kali menginput data Feasibility Study
              </p>
            </div>
            <div class="flex flex-col space-y-1">
      ...
      <div class="flex flex-col space-y-3">
        <div class="flex flex-row items-center justify-between">
          <div class="flex flex-col space-y-1">
            ...
            <p class="text-xs text-textDisabledColor">
              Silahkan unggah Evidence yang berkaitan dengan Feasibility Study
            </p>
          </div>
        </div>
        <div class="flex flex-col space-y-1">
  ...
  <div class="items-start p-6 mt-4 bg-white rounded-lg" v-if="mesin">
    <div v-for="item in approveMesinFS">
      <div v-auto-animate="{ duration: 300 }" v-if="item.status === 'Ditolak T1' || item.status === 'Ditolak T2'"
        ...
        <div class="flex justify-between px-2">
          <div class="flex">
            ...
            <p class="ml-2 font-semibold">Ditolak Unit Pembina</p>
          </div>
        ...
        <p v-if="isHover" v-for="item in approveMesinFS" class="mt-2 ml-8 text-sm capitalize">{{ item.keterangan }}</p>
      </div>
    </div>
    <TabsWrapper :laman-data="false">
      <TabItem :title="'Asumsi Makro'">
        <TabAsumsiMakro :tahun-realisasi="tahunBerjalan" :mesin="mesin.mesin" v-model:interest-rate="interestRate"
          ...
          :error="error.asumsi" :umur-teknis-init="masaManfaat.toString()" />
      </TabItem>
      <TabItem :title="'Parameter Teknis & Finansial'">
        <TabParameterTeknis :tahun-realisasi="tahunBerjalan" :init-pemakaian-sendiri="pemakaianSendiri"
          :init-auxiliary="auxiliary" :init-susut-trafo="susutTrafo" :combo-bahan-bakar="comboBahanBakar"
          :bahan-bakars="bahanBakars" :id_mesin="route.params.id" :is-input-asumsi-parameter="true" :mesin="mesin.mesin"
          v-model:pickedValue="pickedParameterValue" v-model:checkedBahanBakar="checkedBahanBakar" v-model:nphr="nphr"
          v-model:auxiliary="auxiliary" v-model:susut-trafo="susutTrafo" v-model:pemakaian-sendiri="pemakaianSendiri"
    ...
    if (isCreate !== true) {
      if (response.code === 200) {
        ...
        for (const iterator of tempBahanBakars) {
          iterator.harga_bahan_bakar = globalFormat.formatRupiah(iterator.harga_bahan_bakar.toString());
        }
        interestRate.value = globalFormat.formatRupiah(response.data.asumsi_makro.interest_rate.toString());
        umurTeknis.value = masaManfaat.value.toString();
        loanTenor.value = response.data.asumsi_makro.loan_tenor.toString();
        loanPortion.value = globalFormat.formatRupiah(response.data.asumsi_makro.loan_portion.toString());
        nphr.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.nphr.toString());
        auxiliary.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.auxiliary.toString());
        susutTrafo.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.susut_trafo.toString());
        pemakaianSendiri.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.ps.toString());
        electricityPriceA.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.electricity_price_a_rp_per_kwbln.toString());
        electricityPriceB.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.electricity_price_b_rp_per_kwbln.toString());
        electricityPriceC.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.electricity_price_c_rp_per_kwh.toString());
        electricityPriceD.value = globalFormat.formatRupiah(response.data.parameter_teknis_financial.electricity_price_d_rp_per_kwh.toString());
        bahanBakars.value = tempBahanBakars;
        console.log('tes' + pemakaianSendiri.value);
...
const insertAsumsiParameter = async () => {
  try {
    ...
    if (Object.values(errorAsumsiInput).some(value => value === true) || Object.values(errorParameterTeknis).some(value => value === true)) {
      // notifyError('Semua input wajib diisi, mohon cek kembali inputan anda!', 3000);
      ...
      if (idAsumsi.value !== 0) {
        const formAsumsiUpdate = {
          ...
          interest_rate: interestRate.value.includes(',') ? parseFloat(interestRate.value.replace(',', '.')) : parseFloat(interestRate.value),
          umur_teknis: parseInt(masaManfaat.value),
          loan_tenor: parseInt(loanTenor.value),
          loan_portion: loanPortion.value.includes(',') ? parseFloat(loanPortion.value.replace(',', '.')) : parseFloat(loanPortion.value)
        }
        await inputAsumsiParameterService.updateAsumsi(formAsumsiUpdate);
        const finalBahanBakars = bahanBakars.value;
        for (const value of finalBahanBakars) {
          const finalBahanBakar = value.harga_bahan_bakar.includes('.') ? value.harga_bahan_bakar.replace(/[.]/g, '') : value.harga_bahan_bakar;
          value.harga_bahan_bakar = parseFloat(finalBahanBakar.replace(/,/g, '.'))
          value.sfc = parseFloat(value.sfc);
        };
        ...
        const formParameterUpdate = {
          ...
          auxiliary: pickedParameterValue.value === 'auxiliarySusut' ? auxiliary.value.includes(',') ? parseFloat(auxiliary.value.replace(',', '.')) : parseFloat(auxiliary.value) : 0,
          susut_trafo: pickedParameterValue.value === 'auxiliarySusut' ? susutTrafo.value.includes(',') ? parseFloat(susutTrafo.value.replace(',', '.')) : parseFloat(susutTrafo.value) : 0,
          ps: pickedParameterValue.value === 'pemakaianSendiri' ? pemakaianSendiri.value.includes(',') ? parseFloat(pemakaianSendiri.value.replace(',', '.')) : parseFloat(pemakaianSendiri.value) : 0,
...
//         console.log(idAsumsi.value, 'Create');
//         const finalBahanBakars = bahanBakars.value;
//         for (const value of finalBahanBakars) {
//           const finalBahanBakar = value.harga_bahan_bakar.includes('.') ? value.harga_bahan_bakar.replace(/[.]/g, '') : value.harga_bahan_bakar;
//           value.harga_bahan_bakar = parseFloat(finalBahanBakar.replace(/,/g, '.'))
...
const handleDownloadTemplateRekap = async () => {
  try {
    ...
    const fileName = fileNameMatch ? fileNameMatch[1] : `Feasibility Study Actual - ${namaMesin.value}_${tahunBerjalan}_${globalFormat.formatNumberFiveDigits(parseInt(idMesin))}.xlsx`;
