<template>
  ...
  <div class="flex flex-col space-y-4">
    <InfoHeader v-if="mesin" :nama-mesin="mesin.mesin" :nama-pengelola="namaPengelola ? namaPengelola : '-'"
      ...
      <div class="flex flex-row items-center space-x-3">
        ...
        <button
          ...
          @click="currentNamaMesin = mesin.mesin; handleDownloadExcelMesin()" v-if="statusPersetujuan === 'Disetujui'">
    ...
    <div class="flex flex-col space-y-4" v-if="statusPersetujuan === 'Disetujui'">
        ...
        <TabsWrapper :kode-sentral="mesin.kode_sentral" :isLihatGrafik="true" :laman-data="false">
          ...
          <TabItem title="Hasil Simulasi">
            <div class="flex flex-col w-full px-2">
              ...
              <TahunBerjalan @on-click-reload="reloadHasilSimulasi"
                :irr-on-project="hasilSimulasi.now_track_irr_project"
    ...
    <div class="w-full p-4 bg-white rounded-lg" v-else>
      <DraftComponentStatus v-if="statusPersetujuan === 'Draft'" />
      <WaitingT1ComponentStatus v-else-if="statusPersetujuan === 'Menunggu Persetujuan T1'" />
      <WaitingT2ComponentStatus v-else-if="statusPersetujuan === 'Menunggu Persetujuan T2'" />
      <DitolakT1ComponentStatus v-else-if="statusPersetujuan === 'Ditolak T1'" />
      <DitolakT2ComponentStatus v-else-if="statusPersetujuan === 'Ditolak T2'" />
    </div>
  </div>
</template>
...
import DraftComponentStatus from "@/components/ui/DraftComponentStatus.vue";
import WaitingT1ComponentStatus from "@/components/ui/WaitingT1ComponentStatus.vue";
import WaitingT2ComponentStatus from "@/components/ui/WaitingT2ComponentStatus.vue";
import DitolakT1ComponentStatus from "@/components/ui/DitolakT1ComponentStatus.vue";
import DitolakT2ComponentStatus from "@/components/ui/DitolakT2ComponentStatus.vue";
...
const fetchStatusPersetujuan = async () => {
  try {
    ...
    statusPersetujuan.value = response.data.status;
  } catch (error) {
    console.error("Fetch Status Persetujuan Error : ", error);
  }
}
