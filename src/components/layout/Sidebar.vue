<template>
  ...
  <Teleport :to="'body'">
    <Transition name="modal">
      <div ref="modal-backdrop" :class="`fixed z-[999] inset-0 overflow-y-auto bg-black bg-opacity-50`"
        v-if="isShowLogoutNotification" @keydown.esc="isShowLogoutNotification = false">
        <div :class="`min-h-screen flex items-center justify-center text-white`">
          Anda akan otomatis logout dalam hitungan {{ timeLeft }} karena tidak ada aktivitas...
        </div>
      </div>
    </Transition>
  </Teleport>
    ...
    <div v-show="isSidebarOpen" class="h-full pb-4 overflow-y-auto bg-white whitespace-nowrap">
      <ul class="mt-5 space-y-5 font-medium">
        <li v-for="(menuItem, menuIndex) in menuList" :key="menuIndex" class="px-3">
          <div class="flex items-center px-2 text-[#7F7F80] rounded-lg">
            <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900" width="20" height="20"
              viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="menuItem.menu === 'Beranda'">
              <path fill-rule="evenodd" clip-rule="evenodd"
            ...
            <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900" width="22" height="22"
              viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" v-else-if="menuItem.menu === 'Data'">
              <path fill-rule="evenodd" clip-rule="evenodd"
            ...
            <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900" width="22" height="22"
              ...
              v-else-if="menuItem.menu === 'Verifikasi'">
              <path
            ...
            <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900" width="22" height="22"
              viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" v-else-if="menuItem.menu === 'Master'">
              <path
            ...
            <svg class="w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900 " width="21" height="21"
              viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
              v-else-if="menuItem.menu === 'Manajemen'">
              <path fill-rule="evenodd" clip-rule="evenodd"
            ...
            <span class="mt-1 ml-3 text-sm font-semibold">{{ menuItem.menu }}</span>
          </div>
          <ul class="mt-1.5 space-y-1.5">
            <template
              v-if="menuItem.sub_menus.some(item => item.is_docked && !['Laman Utama', 'Laman Data', 'Laman Analitik'].includes(item.sub_menu))">
              <li
                ...
                <RouterLink :to="`/${subMenuItem.url}`" id="sidebar-button" @click="store.label = subMenuItem.sub_menu"
                  ...
                  <div class="flex items-center ml-5 text-xs">
                    <p class="mr-2">{{ subMenuItem.sub_menu }}</p>
                    <div class="bg-[#FF6363] text-white px-1 py-0.5 rounded-sm"
                      v-if="subMenuItem.sub_menu === 'Persetujuan'">{{ totalPersetujuanKK +
                        totalPersetujuanFS }}
                    </div>
                  </div>
                </RouterLink>
            ...
            <template
              v-if="menuItem.sub_menus.some(item => ['Laman Utama', 'Laman Data', 'Laman Analitik'].includes(item.sub_menu))">
              <div class="px-3 mt-1 mb-2">
                <div class="flex items-center px-2 pt-1 text-[#7F7F80]">
                  <span class="ml-5 text-xs font-semibold">Laman</span>
                </div>
              </div>
              <li
                ...
                <RouterLink :to="`/${subMenuItem.url}`" id="sidebar-button" @click="store.label = subMenuItem.sub_menu"
                  ...
                  <span class="ml-3 text-xs">{{ subMenuItem.sub_menu }}</span>
                </RouterLink>
              </li>
            </template>
          </ul>
...
const timeLeft = computed(() =>
  Math.max(0, 15 * 60 - Math.floor((now.value - lastActive.value) / 1000))
); // Sisa waktu dalam detik
const isShowLogoutNotification = ref<boolean>(false);

const startCountdown = () => {
  countdown.value = 10; // Mulai dari 10 detik
...
watch(timeLeft, async (remainingTime) => {
  const encryptStorage = await encryptStoragePromise;
  if (nodeMode === 'production') {
    if (remainingTime === 10) { // 10 detik sebelum timeout
      isShowLogoutNotification.value = true;
      startCountdown();
    } else if (remainingTime > 10 && isShowLogoutNotification.value) {
      isShowLogoutNotification.value = false;
      clearInterval(timer.value!);
      timer.value = null;
    }
    if (remainingTime === 0) { // Waktu habis
      try {
